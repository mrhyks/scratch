...
int main(int argc, char *argv[]) {
  ...
  //lteHelper -> SetEnbDeviceAttribute("DlBandwidth",UintegerValue(100)); // Cambiamos el ancho de banda del sistema
  //lteHelper -> SetEnbDeviceAttribute("UlBandwidth",UintegerValue(100)); // El nombre tiene que ser exacto ese
  ...
  Ptr<NetDevice> enbNetDev = enbLteDevs.Get(0); // Solo se modifica el del enb 0
  Ptr<LteEnbNetDevice> enbLteNetDev = DynamicCast<LteEnbNetDevice>(enbNetDev); // Solo hay que hacer una de las dos formas de configurar, estas 2 lineas o las 2 de debajo
  // enbLteNetDev -> SetDlBandwidth(100);
  // enbLteNetDev -> SetUlBandwidth(100);

  // Ptr<NetDevice> enbNetDev = enbLteDevs.Get(1); // Copiamos el bloque de arriba para el segundo e node b cambiando el indice
  ...
  uint16_t dlBandwidth = enbLteNetDev -> GetDlBandwidth();
  uint16_t ulBandwidth = enbLteNetDev -> GetUlBandwidth();
  ...
  std::cout << "DL Earfcn: " << dlEarfcn << std::endl 
            ...
            << "DL Bandwidth: " << dlBandwidth << std::endl // Da 25 que es el valor de Resource Block
            << "UL Bandwidth: " << ulBandwidth << std::endl // Da 25 que es el valor de Resource Block
            << "TxPower eNodeB: " << txPower << std::endl; 
  ...
  double txPowerUE = enbPhy ->GetTxPower();
  std::cout << "TxPower UE: " << dlEarfcn << std::endl; 
  ...
  // Create UDP Client  as 2nd client application
  uint16_t port2 = 33;
  UdpClientHelper ulClient (remoteHostAddr, port2);
  ulClient.SetAttribute("Interval", TimeValue(MilliSeconds(interval)));
  ...
  for (uint16_t i = 0; i < numberOfUes; i++)
    {
      if (i < numberOfUes/2) {
        ...
        sourceApps2.Add(ulClient.Install(ueNodes.Get(i)));
    }
